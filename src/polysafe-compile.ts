...
async function main() {
  ...
  const sourceCode = await fs.readFileSync('./contracts/Polysafe.sol', 'utf8');
  // Compile the source code and retrieve the ABI and Bytecode
  const output = compileSol(sourceCode, 'Polysafe');
  if (output) {
    const { abi, bytecode } = output;
    // Store the ABI and Bytecode into a JSON file
    const artifact = JSON.stringify({ abi, bytecode }, null, 2);
    fs.writeFileSync('./contracts/Polysafe.json', artifact);
  } else {
